...
//#define SOURCE_JPG  "D:/biaoji/sf6new/基准图.bmp"
...
int main(int argc, char* argv[])
{
	...
	char *imgfile="D:/biaoji/sf6new/匹配模板.jpg";
	//char *peizhifile="D:/z/biaoji.mat";
	//char *imgfile="D:/z/匹配模板.jpg";
	srcImage = cvLoadImage(SOURCE_JPG, CV_LOAD_IMAGE_COLOR);
	src = cvLoadImage(SOURCE_JPG, CV_LOAD_IMAGE_COLOR);
	baseImage = cvLoadImage("D:/biaoji/sf6new/基准图.jpg", CV_LOAD_IMAGE_COLOR);
...
//	Sleep(2000);
	...
	//MeterRead_Recog(&img,wy_maskImage,para, &mParam,MResult);  //setMaskPicIn没更新
...
int MeterReadTrain_data(HYAMR_INTERACTIVE_PARA *&pOutPattern, MaskParam *pMaskParam){
	...
	int lTmpMin=0;//最小值
	WY_DEBUG_PRINT("每个点对应的刻度值\n");
	for (int i=0; i<pOutPattern->lPtNum; i++)
	{
		pOutPattern->dPtValList[i] = (MDouble)lTmpMin;//每个点对应的刻度值
		printf("pOutPattern->dPtValList[%d] = %d\n",i,lTmpMin);
		lTmpMin=lTmpMin+step;
	}

	WY_DEBUG_PRINT("圈定圆心\n");
	if(pOutPattern->lPtNum>2)//圈定圆心
	{
...
//	cvNamedWindow("testImage",0);
	...
	/*
	int value=0;
	cvCreateTrackbar("指针线条数","TrainImage",&value,2,track_bar);
	cvWaitKey();
	value=0;
	if(cvGetTrackbarPos("指针线条数","TrainImage")==1)
	{
		cvCreateTrackbar("指针线颜色","TrainImage",&value,4,track_bar);
	}
	else
	{
		cvCreateTrackbar("指针线颜色","TrainImage",&value,3,track_bar);
	}
	cvWaitKey();
	value=0;
	cvCreateTrackbar("白色表盘","TrainImage",&value,1,track_bar);
	cvWaitKey();
	value=0;
	cvCreateTrackbar("表盘指针粗细","TrainImage",&value,1,track_bar);
	cvWaitKey();
	value=0;
	cvCreateTrackbar("表盘MASK类型","TrainImage",&value,1,track_bar);
	cvWaitKey();
	*/
	WY_DEBUG_PRINT("表盘中指针线有几条(1/2?)\n");
	scanf("%d", &lTmpVal);
	...
	{
		/*int value=0;
		cvCreateTrackbar("指针线宽度","TrainImage",&value,100,track_bar);
		cvWaitKey();
		pOutPattern->lLineWidth = cvGetTrackbarPos("指针线宽度","TrainImage");
	...
	//fprintf(fp, "%ld ", pOutPattern->lLineWidth);
	WY_DEBUG_PRINT("请设计表盘MASK类型：（1.矩形 2.圆形 3.圆环 4.圆环+矩形 5.圆形+矩形 6.圆形+2个矩形 7.圆形+3个矩形 8.圆环+3矩形 9.圆环+6矩形 10.圆环+9矩形 11.3圆环+圆心 12.其它）\n");
	scanf("%d", &lTmpVal);
	switch(lTmpVal)
	{
		case 1:
