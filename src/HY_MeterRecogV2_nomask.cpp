...
int MeterReadRecogV2(HYLPAMR_IMAGES src,MeterDESCRIP Meterdescrip,HYAMR_Meter_Result *MeterResult){
	...
	if(src->lPixelArrayFormat==HYAMR_IMAGE_BGR){
		...
		/*int Hstart=Meterdescrip.MeterRect.top-2*QRH;  // old
		int Hend=Meterdescrip.MeterRect.bottom;
		int Wstart=Meterdescrip.MeterRect.left-QRW/2;
		int Wend=Meterdescrip.MeterRect.right+QRW/2;*/
	...
	if(Meterdescrip.MeterType==1){
		...
		//peizhifile="D:/biaoji/sf6/biaoji.mat";
		//imgfile="D:/biaoji/sf6/匹配模板.jpg";
		peizhifile="./biaoji/sf6/biaoji.mat";  //标准路径
