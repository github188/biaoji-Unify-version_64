...
MRESULT HYAMR_GetLineParamNoMem(MHandle hHYMRDHandle, HYLPAMR_IMAGES pSrcImage, HYAMR_INTERACTIVE_PARA *pPara, HYAMR_READ_PARA *pLinePara)
{
	MRESULT res = LI_ERR_NONE;
	...
	for (i=0; i<pPara->lLineNum; i++)
	{
		...
		else if (HY_LINE_YELLOW == pPara->lLineColor[i])		// 增加黄色指针开关的case
		{
			...
			{		
				...
				{
					if(pBlockData[j*srcBlock.lBlockLine+i] >120)   //黄色偏亮 统计区域内亮色像素的个数来判断指针的位置
					{
			...
			if(YellowNum1>YellowNum2)  //黄色指针线在上面 反之在下面  然后人为制作输入图像
			{
				for (j=0;j<srcBlock.lHeight;j++)
				{		
					for(i =0; i<srcBlock.lWidth;i++)    
...
MRESULT HYAMR_SetParam(MHandle hHYMRDHandle, HYLPAMR_IMAGES pMaskImage, HYAMR_INTERACTIVE_PARA *pInPara)
{
	...
	MLong lRate;
	MLong tmpR=0;
	pMgr->ptTarget.left=pMgr->ptTrainTarget.left=pInPara->startPt.x;
	pMgr->ptTarget.right=pMgr->ptTrainTarget.right=pInPara->endPt.x;
	pMgr->ptTarget.top=pMgr->ptTrainTarget.top=pInPara->startPt.y;
	pMgr->ptTarget.bottom=pMgr->ptTrainTarget.bottom=pInPara->endPt.y;
	//printf("ptTarget:left=%d  right=%d  top=%d  bottom=%d\n", pMgr->ptTarget.left, pMgr->ptTarget.right, pMgr->ptTarget.top, pMgr->ptTarget.bottom);
	//printf("ptTrainTarget:left=%d  right=%d  top=%d  bottom=%d\n", pMgr->ptTrainTarget.left, pMgr->ptTrainTarget.right, pMgr->ptTrainTarget.top, pMgr->ptTrainTarget.bottom);
	if (pMgr==MNull)
	{
		pMgr->bExistCircle = MFalse;
