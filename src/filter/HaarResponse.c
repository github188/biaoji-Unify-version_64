...
MRESULT GetLineInfo(MHandle hMemMgr,BLOCK *pImage, BLOCK *pMask, PARAM_INFO *pParam)
{
	...
	//获取响应图
	sprintf(gPicName,"D:\\gray_aaa.bmp",gPictureNum);//ls
	...
	sprintf(gPicName,"D:\\response_aaa.bmp",gPictureNum);//ls
	...
	sprintf(gPicName,".\\%d_pmask.bmp",gPictureNum);//ls
	...
	sprintf(gPicName,"D:\\response_bbb.bmp",gPictureNum);//ls
	PrintBmpEx(haarResult.pBlockData, haarResult.lBlockLine, DATA_U8, haarResult.lWidth, haarResult.lHeight, 1, gPicName);

	if (PIC_HIGH_BLUR == pParam->lPicBlurLevel)
	{
		inverseImg(&haarResult);
		sprintf(gPicName,"D:\\response_ccc.bmp",gPictureNum);
		PrintBmpEx(haarResult.pBlockData, haarResult.lBlockLine, DATA_U8, haarResult.lWidth, haarResult.lHeight, 1, gPicName);
	...
	if(pParam->lDoubleRedLine)
	{
		...
		sprintf(gPicName,"D:\\response_ddd.bmp",gPictureNum);
		PrintBmpEx(haarResult.pBlockData, haarResult.lBlockLine, DATA_U8, haarResult.lWidth, haarResult.lHeight, 1, gPicName);
	}
	...
	GO(LocalMax_Circle(hMemMgr, (MByte*)haarResult.pBlockData, haarResult.lBlockLine, (MByte *)blockFlag.pBlockData,blockFlag.lBlockLine,
		blockFlag.lWidth,blockFlag.lHeight,pTemp,lStackLen, lThreshold, (MLong)(lThreshold*0.6), 50, 128, &seedsTemp, lHaarWidth>>1));	//0.6  这个的lHaarWidth适当减少
	...
	ridgePoint(hMemMgr,(MByte*)haarResult.pBlockData,haarResult.lBlockLine, haarResult.lWidth, haarResult.lHeight,
		...
		(MLong)(lThreshold*0.6), 0, 250, &seedsTemp);     //增加种子点的个数 不然又可能会出现有效区域种子点个数不够的情况

	sprintf(gPicName, "D:\\localMaxPre_bbb.bmp",gPictureNum);//ls
	...
	//printf("ptEnd.x=%d ptEnd.y=%d\n ",pLineParam->ptEnd.x,pLineParam->ptEnd.y);
	//printf("ptStart.x=%d ptStart.y=%d\n ",pLineParam->ptStart.x,pLineParam->ptStart.y);
	CalcLineEndPt_xsd(hMemMgr, &haarResult, pParam->pCircleInfo->circleParam.lRadius, lThreshold, pLineParam);
	//printf("ptEnd.x=%d ptEnd.y=%d\n ",pLineParam->ptEnd.x,pLineParam->ptEnd.y);
	//printf("ptStart.x=%d ptStart.y=%d\n ",pLineParam->ptStart.x,pLineParam->ptStart.y);

	/*vDrawLine2((MByte*)blockFlag.pBlockData,blockFlag.lBlockLine,blockFlag.lWidth, blockFlag.lHeight,255,pLineParam->Coefk,pLineParam->Coefb);
	sprintf(gPicName, "D:\\localMaxPost_line.bmp",gPictureNum);
	PrintBmpEx(blockFlag.pBlockData, blockFlag.lBlockLine, DATA_U8, blockFlag.lWidth, blockFlag.lHeight, 1,gPicName);*/
EXT:
	...
	MPOINT *pTemp,MLong lTmpNum, PARAM_INFO *pParam)
{
	...
	//	JPrintf("CreateLineSet~~~\n");
	{
		...
		if(pCircleInfo->circleParam.xcoord !=-10000)
		{
			...
			maxDist2Center = MAX(5, (pCircleInfo->circleParam.lRadius>>2));
		}
		else
		{
		...
		for (j=0; j<seedTmp.lSeedNum; j++)
		{
			...
			for (k=j+1; k<seedTmp.lSeedNum; k++)
			{
				...
				randomPoint[1].x = seedTmp.pptSeed[k].x;
				randomPoint[1].y = seedTmp.pptSeed[k].y;
				if(disTmp>= pParam->lCircleDist<<1)    //圆环两边种子点不能相连
					continue;	
				...
				while(bChanged)
				{
					...
					for (m = 0 ; m<seedTmp.lSeedNum; m++)
					{
						...
						if (distance < DISTANCE_THRESHOLD)
						{
							...
							{
								dis = 0;
								for(n = 0;n<numInPoint;n++)          //圆环情况 圆环两边的种子点不能相连
								{
				...
				MChar path[256]={0};
				printf("产生了一条直线[%d]\n",tmp);
				sprintf(path, "D:\\line\\linepos_%d.bmp", tmp);
				tmp++;
				...
				vDrawLine3((MByte*)blockFlagTmp1.pBlockData, blockFlagTmp1.lBlockLine, blockFlagTmp1.lWidth, blockFlagTmp1.lHeight, 
					255, dCoeffK, dCoeffB, bVert);
				PrintBmpEx(blockFlagTmp1.pBlockData, blockFlagTmp1.lBlockLine, DATA_U8, blockFlagTmp1.lWidth, blockFlagTmp1.lHeight, 1,path);
				}
				printf("distance=%lf maxDist2Center=%d lLineNum=%d lMaxLineNum=%d\n",distance,maxDist2Center,lineSetInfo.lLineNum,lineSetInfo.lMaxLineNum);
				/*******************/
				if (distance < maxDist2Center &&  lineSetInfo.lLineNum < lineSetInfo.lMaxLineNum)
				{
					lineSetInfo.pLineInfo[lineSetInfo.lLineNum].lineParam.Coefk = dCoeffK;
		...
		for (j=0; j<lineSetInfo.lLineNum; j++)
		{
			...
			//pLineInfo->lConfidence = pLineInfo->lMaxLength/distance;
			{	
				/*MChar path[256]={0};//ls
				sprintf(path, "D:\\line\\line1\\line_%d_%d.bmp", j,pLineInfo->lMaxLength);
				B_Cpy(&blockFlagTmp1, pHaarResponse);
				vDrawLine3((MByte*)blockFlagTmp1.pBlockData, blockFlagTmp1.lBlockLine, blockFlagTmp1.lWidth, blockFlagTmp1.lHeight, 
					255, pLineInfo->lineParam.Coefk, pLineInfo->lineParam.Coefb, pLineInfo->lineParam.bVertical);
				PrintBmpEx(blockFlagTmp1.pBlockData, blockFlagTmp1.lBlockLine, DATA_U8, blockFlagTmp1.lWidth, blockFlagTmp1.lHeight, 1,path);*/
			}
					//	printf("index=%d, line:k=%.4f, b=%.4f, con=%d, sum=%d, lPtNum=%d, lPtNum2=%d, lMaxLength=%d\n", j, pLineInfo->lineParam.Coefk, pLineInfo->lineParam.Coefb,
					//			pLineInfo->lConfidence, lMeanSum, lTotalPtNum, lTotalSumNum,pLineInfo->lMaxLength);
		}
		GO(QuickSort(hMemMgr, (MVoid *)(lineSetInfo.pLineInfo), lineSetInfo.lLineNum, sizeof(LINE_INFO), LineInfoConfidenceCmp));
		//如果没有直线，这个confidence为0
		...
		for (j=0; j<lineSetInfo.lLineNum; j++)
		{
			...
			//distance = vPointDistToLine2(randomPoint[2],  pLineInfo->lineParam.Coefk, pLineInfo->lineParam.Coefb, pLineInfo->lineParam.bVertical);
			//printf("line2\\line_%d:lConfidence=%d, lMaxLength=%d, Coefk=%f,Coefb=%f\n",j,pLineInfo->lConfidence,pLineInfo->lMaxLength,pLineInfo->lineParam.Coefk, pLineInfo->lineParam.Coefb);
		...
		//直线与圆的交点
		{
			...
			ptMid.y = (ptCross1.y+ptCross2.y)>>1;
			/***************微调******************/
			//printf("Coefk=%f Coefb=%f\n",lineSetInfo.pLineInfo[lIndex].lineParam.Coefk,lineSetInfo.pLineInfo[lIndex].lineParam.Coefb);
			AdjustLinePosition((MByte*)pHaarResponse->pBlockData, pHaarResponse->lBlockLine,
				pHaarResponse->lWidth, pHaarResponse->lHeight, 
			...
			{	
				/*MChar path[256]={0};//ls
				LINE_INFO *pLineInfo = &(pCircleInfo->lineInfo);
				sprintf(path, "D:\\line.bmp",gPictureNum);
				B_Cpy(&blockFlagTmp1, pHaarResponse);
				vDrawLine3((MByte*)blockFlagTmp1.pBlockData, blockFlagTmp1.lBlockLine, blockFlagTmp1.lWidth, blockFlagTmp1.lHeight, 
					255, pLineInfo->lineParam.Coefk, pLineInfo->lineParam.Coefb, pLineInfo->lineParam.bVertical);
				PrintBmpEx(blockFlagTmp1.pBlockData, blockFlagTmp1.lBlockLine, DATA_U8, blockFlagTmp1.lWidth, blockFlagTmp1.lHeight, 1,path);*/
	...
	LINEPARAM lineParam, MLong minLength, MLong *leftWidthConfidence, MLong *rightWidthConfidence)
{
	...
	if(0)          //改进宽度置信度算法,添加权重，为点到中点的距离      2018/06/05  ls
	{
		...
		//printf("leftWidthConfidence=%d lWidthStep=%d\n",(*leftWidthConfidence),lWidthStep);
	...
	MPOINT ptCross1, MPOINT ptCross2, MLong lMask)
{
		...
		else
		{
			...
			for (yy=lStart; yy<=lEnd; yy++)
			{
				...
				else
				{
					...
					{
						lMaxLength = lStep;
						Length = sqrt((MDouble)(lMaxLength/k)*(lMaxLength/k)+lMaxLength*lMaxLength) ;  //输出的length是表计的实际长度不是投影值
					}
					lStep = 0;
				}
			}
	...
	if (lMaxLength < lStep)    ///在循环外要添加这一步 不然当循环每个响应点都有响应时 Maxlength一直未被赋值 是bug
	{
		lMaxLength = lStep;
		Length = sqrt((MDouble)(lMaxLength/k)*(lMaxLength/k)+lMaxLength*lMaxLength) ;
	}
