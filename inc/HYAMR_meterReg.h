...
typedef struct{
	MRECT MeterRect;//仪表在图片中位置
	int MeterType;//仪表类型
}MeterDESCRIP;//仪表坐标位置和仪表类型
...
typedef struct{
	...
	MDouble MeterValue;//仪表读数
}HYAMR_Meter_Result;//仪表分析结果
...
METERREG_API MRESULT HYAMR_GetLineParamNoMem(MHandle hHYMRDHandle, HYLPAMR_IMAGES pSrcImage, HYAMR_INTERACTIVE_PARA *pPara,
										HYAMR_READ_PARA *pLinePara);
